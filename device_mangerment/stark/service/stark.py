...
class Option(object):
    ...
    def get_queryset(self, _field, model_class, config):
        if isinstance(_field, ForeignKey) or isinstance(_field, ManyToManyField):
            ...
            # row = Row(_field.remote_field.model.objects.filter(**self.condition), self, config.request.GET)  dango2.0
        else:
            if self.is_choice:
...
class StarkConfig(object):
    ...
    def changelist_view(self, request, *args, **kwargs):
        ...
        page = Pagination(request.GET.get('page'), total_count, request.path_info, query_params, per_page=15)

        print(">>>>>",page.start,page.end)
        # ##################################
        # from app01.utils import dome_ping
        # host_list_quert= self.model_class.objects.filter(id__lt=page.end,id__gt=page.start).values()
        # print(">>>>host_list_quert",host_list_quert)
        #
        # hosts = []
        # host_staus = []
        ...
        #         if  v:
        #             self.model_class.objects.filter(hostname=k).update(device_status_id=1)
        #         else:
        #             self.model_class.objects.filter(hostname=k).update(device_status_id=2)
        ###############################
        # ##### 获取组合搜索筛选 #####
